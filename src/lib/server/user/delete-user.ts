 ...
 * @throws {Error} If the MAILER_EMAIL_USER environmental variable is missing.
 */
async function sendAccountDeleteEmail(params: {
  ...
  const from = process.env.MAILER_EMAIL_USER;

  if (!from) {
    throw new Error(`Missing MAILER_EMAIL_USER env variable.`);
  }
